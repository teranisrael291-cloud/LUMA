(() => {
  ...
  function renderPublished() {
    ...
    el.innerHTML = `<div><span>${esc(p.meta?.issue || '')} · ${esc(p.meta?.updated || '')}</span><br><b>${esc(p.editorial?.headline || 'LUMA Pulse')}</b><br><span>${p.trends?.length || 0} señales publicadas</span></div><a href="pulse.html" target="_blank">Ver edición ↗</a>`;
  }
  ...
  function renderEngine() {
    ...
    summary.innerHTML = stats.map(([label,value]) => `<div class="engine-stat"><b>${esc(value)}</b><span>${esc(label)}</span></div>`).join('');
  ...
  function renderList() {
    ...
    list.innerHTML = trends.length ? trends.map(t => `<article class="candidate-card ${state.selected.has(t.id)?'selected':''}" data-id="${esc(t.id)}">
      ...
      <div class="candidate-copy"><div class="candidate-meta"><span>${esc(t.region)}</span><span>${esc(t.segment)}</span><span>${esc(t.audience || 'Both')}</span><span>${esc(t.stage)}</span></div><h3>${esc(t.title)}</h3><p>${esc(t.summary)}</p><div class="candidate-source">${esc(t.source)} · ${esc(t.metric)}</div></div>
      <div class="candidate-score"><b>${Number(t.score)||0}</b><button type="button" class="edit-trend">Editar</button></div></article>`).join('') : '<div class="empty-editor">No hay señales en este filtro.</div>';
  ...
  function openEditor(id) {
    ...
    editor.innerHTML = `<form class="trend-form" id="trend-form"><div class="trend-form-head"><div><span class="admin-kicker">EDIT SIGNAL</span><h3>${esc(t.title)}</h3></div><button type="button" id="close-editor" aria-label="Cerrar">×</button></div>
      <div class="trend-form-grid">
        ...
        <label>Stage<select name="stage">${['Spark','Rising','Accelerating','Building','Mainstream','Cooling','Shift'].map(x=>`<option ${t.stage===x?'selected':''}>${x}</option>`).join('')}</select></label>
      ...
      <div class="inline-note"><b>ML/NLP trace:</b> momentum ${esc(t.model?.momentum ?? '—')} · evidence ${esc(t.model?.evidence ?? '—')} · authority ${esc(t.model?.authority ?? '—')} · age ${esc(t.model?.ageDays ?? '—')}d. El summary inicial es extractivo (TextRank), no generativo.</div>
    ...
    $('#close-editor').addEventListener('click', () => { state.editingId = null; editor.innerHTML = '<div class="empty-editor"><span>✦</span><p>Selecciona “Editar” en una señal para ajustar su narrativa antes de publicar.</p></div>'; });
    $('#trend-form').addEventListener('submit', e => {
      e.preventDefault(); const f = new FormData(e.currentTarget);
      ['title','kicker','region','audience','stage','audienceWhy','summary','impact','creator','watchout','metric','metricLabel','source','platform','sourceUrl'].forEach(k => t[k] = String(f.get(k)||'').trim());
